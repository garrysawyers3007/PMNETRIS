...
from network.pmnet import PMNet, PMNetFiLM, PMNetFiLMNew, PMNetFilMModified, PMNetSpatialFiLM, PMNetBaseline, PMNet4Ch
from network.rismapnet import RISMapNet, RISMapNetConfig
from config import config_USC_pmnetV3_V2, config_USC_RISMapNet_V1, config_USC_pmnetRIS_V1
...
def canonical_group_key(sample_meta: dict) -> str:
    """
    Group each RIS sample with its paired noRIS sample at the same
    augmentation index, but allow different augmentation indices
    to land in different splits.
    """
    if sample_meta["type"] == "RIS":
        path = sample_meta["noris_power_map_path"]
    else:
        path = sample_meta["power_map_path"]

    base = os.path.basename(path)
    # e.g. 83_noRIS_rx2_power_map_3.png -> 83_noRIS_rx2_power_map_3
    key = _EXT_RE.sub("", base)
    return key


def grouped_split_from_flat_metadata(metadata_records, train_ratio=0.9, seed=1234):
    groups = defaultdict(list)

    for idx, rec in enumerate(metadata_records):
        key = canonical_group_key(rec)
    ...
    n_train_groups = int(len(group_keys) * train_ratio)
    train_keys = set(group_keys[:n_train_groups])
    test_keys = set(group_keys[n_train_groups:])
...
def verify_no_leakage(metadata_records, train_indices, test_indices):
    train_keys = {canonical_group_key(metadata_records[i]) for i in train_indices}
    test_keys = {canonical_group_key(metadata_records[i]) for i in test_indices}
    overlap = train_keys & test_keys
    print("Overlapping noRIS families:", len(overlap))
    if overlap:
...
def train(model, train_loader, test_loader, optimizer, scheduler, writer, cfg=None):
    ...
    for epoch in range(cfg.num_epochs):
        ...
        for inputs, ris_params, _, ris_polar_tensor, targets, noris_targets in tqdm(train_loader):
            count += 1

            inputs = inputs.cuda()
            # ris_params = torch.cat([ris_params[:, :6], ris_params[:, 9:10]], dim=1)
            # ris_params = ris_params.cuda()
            ris_polar_tensor = ris_polar_tensor.cuda()
            ...
            optimizer.zero_grad()
            # preds = model(inputs)
            preds = model(inputs, ris_polar_tensor)
            noris_cond = torch.zeros_like(ris_polar_tensor)
            pred_noris = model(inputs, noris_cond)

            # loss = RMSE(preds, targets)
            # loss = L1_loss(preds, targets)
            loss = ris_gain_loss(preds, targets, pred_noris, noris_targets, lam=0.4)
...
def eval_model(model, test_loader, error="RMSE", best_val=100, cfg=None, eval_mode=False, infer_img_path=''):
    ...
    for inputs, ris_params, _, ris_polar_tensor, targets, __ in tqdm(test_loader):
        inputs = inputs.cuda()
        # ris_params = torch.cat([ris_params[:, :6], ris_params[:, 9:10]], dim=1)
        # ris_params = ris_params.cuda()
        ris_polar_tensor = ris_polar_tensor.cuda()
...
def helper(cfg, writer, data_root = '', load_model=''):
    ...
    if cfg.sampling == 'exclusive':
        ris_pos_min = [-400, -400, 0]  # Example bounds for RIS position normalization
        ris_pos_max = [400, 400, 55]  # Example bounds for RIS position normalization
        ...
        data_usc_train = PMnet_data_usc(dir_dataset=data_root, 
                             ...
                             ris_pos_max=ris_pos_max, use_ris_map=False)

        dataset_size = len(data_usc_train)

        train_indices, test_indices, groups = grouped_split_from_flat_metadata(
            data_usc_train.metadata_records,
            train_ratio=cfg.train_ratio,
            seed=1234
        )

        train_dataset = Subset(data_usc_train, train_indices)
        test_dataset = Subset(data_usc_train, test_indices)

        print(f"Train samples: {len(train_indices)}")
        print(f"Test samples: {len(test_indices)}")
        print(f"Groups: {len(groups)}")

        verify_no_leakage(data_usc_train.metadata_records, train_indices, test_indices)
...
if __name__ == "__main__":
    ...
    data_root = "datasetGeoRIS_collective/"  # Change this to your dataset directory
    RESULT_FOLDER = f'{data_root}results_L1_loss_vec_film_polar_raw_origin_lam0.4_new_split'

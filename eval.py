    ...
    for inputs, ris_params, targets in tqdm(test_loader):
        ...
        with torch.set_grad_enabled(False):
            if error == "MSE":
                ...
                criterion = L1_loss
            elif error == "roi_rmse_loss":
                criterion = roi_rmse_loss

            preds = model(inputs, ris_params)
            preds = torch.clip(preds, 0, 1)
            
            if error == 'roi_rmse_loss':
                loss = criterion(inputs*255, preds, targets)
            else:
            ...
            if infer_img_path!='':
                for i in range(len(preds)):
                    ...
                    ris_params_np = ris_params[i].cpu().detach().numpy()
...
def helper(cfg, data_root = '', load_model=''):
    ...
    result = eval_model(model, test_loader, error="roi_rmse_loss", best_val=100, cfg=None, eval_mode=True,

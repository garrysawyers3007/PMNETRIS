...
class PMnet_data_usc(Dataset):
    ...
    def _process_metadata(self, original_records):
        ...
        for record_idx, record in enumerate(original_records):
            ...
            if not isinstance(paths_dict, dict) or \
               not all(k in paths_dict for k in ["city_map", "tx_map", "power_map", "rx_map"] + (["ris_map"] if self.use_ris_map else [])):
            ...
            for i in range(num_augmentations): # Typically 0, 1, 2, 3
                if not (len(paths_dict["tx_map"]) > i and len(paths_dict["power_map"]) > i and
                        (not self.use_ris_map or len(paths_dict["ris_map"]) > i)):
                ...
                noris_power_map_list = record.get("noRIS_power_map")
                if isinstance(noris_power_map_list, list) and len(noris_power_map_list) > i:
                    data_point["noris_power_map_path"] = noris_power_map_list[i]
                else:
                    print(f"Warning: Missing 'noRIS_power_map' for augmentation index {i} in record {record_idx}. Skipping this augmentation.")
                    continue
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            if self.use_ris_map:
                image_ris = np.asarray(io.imread(data_point["ris_map_path"]))
                if image_ris.ndim == 3 and image_ris.shape[-1] >= 3:
                    image_ris = image_ris[:,:,0]
            ...
            image_noris_power = np.asarray(io.imread(data_point["noris_power_map_path"]))
            if image_noris_power.ndim == 3 and image_noris_power.shape[-1] >= 3:
                image_noris_power = image_noris_power[:,:,0]
        ...
        if self.use_ris_map:
            inputs_np = np.stack([image_buildings, image_tx, image_rx, image_ris], axis=-1)
        else:
        ...
        noris_power_tensor_np = image_noris_power

        if self.transform:
            inputs_tensor = self.transform(inputs_np).type(torch.float32)
            
            _to_14 = transforms.Resize((14, 14), antialias=True)

            if power_tensor_np.dtype == np.uint8:
                power_tensor = _to_14(transforms.ToTensor()(power_tensor_np)).type(torch.float32)
            else: 
                power_tensor = _to_14(transforms.ToTensor()(power_tensor_np.astype(np.uint8))).type(torch.float32)

            if noris_power_tensor_np.dtype == np.uint8:
                noris_power_tensor = _to_14(transforms.ToTensor()(noris_power_tensor_np)).type(torch.float32)
            else:
                noris_power_tensor = _to_14(transforms.ToTensor()(noris_power_tensor_np.astype(np.uint8))).type(torch.float32)

        if self.get_paths:
            return inputs_tensor, ris_info_tensor, ris_polar_tensor, ris_rho_theta_phi_tensor, power_tensor, noris_power_tensor, data_point["city_map_path"], data_point["tx_map_path"], data_point["power_map_path"]
            
        return inputs_tensor, ris_info_tensor, ris_polar_tensor, ris_rho_theta_phi_tensor, power_tensor, noris_power_tensor


class PMnet_data_usc_256(Dataset):
    def __init__(self,
                 dir_dataset="", # Path to the root directory containing metadata.json (e.g., "datasetRIS/")
                 transform=transforms.Compose([
                     transforms.ToTensor(),
        ...
        self.get_paths = get_paths
        self.use_ris_map = use_ris_map
        self.only_ris_records = only_ris_records
    ...
    def _process_metadata(self, original_records):
        processed_list = []
        for record_idx, record in enumerate(original_records):
            if self.only_ris_records and record.get("type") != "noRIS":
                continue
